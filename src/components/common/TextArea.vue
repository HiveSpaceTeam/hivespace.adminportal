<template>
  <div :class="[
    ...
    <div class="flex-1">
      <textarea :id="id" :value="modelValue" @input="onInput" @focus="onFocus" @blur="onBlur" :placeholder="placeholder"
        :rows="rows" :disabled="disabled" :required="required" :class="computedClass" v-bind="$attrs"></textarea>
...
const disabledClass = 'disabled:border-gray-100 disabled:bg-gray-50 disabled:placeholder:text-gray-300 dark:disabled:border-gray-800 dark:disabled:bg-white/[0.03] dark:disabled:placeholder:text-white/15'

const normalClass = 'border-gray-300 focus:border-brand-300 focus:ring-brand-500/10 dark:border-gray-700 dark:focus:border-brand-800'
