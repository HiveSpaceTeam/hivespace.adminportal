...
const changeLanguage = async (lang) => {
  ...
  if (user) {
    // For authenticated users: update store (which handles API call and i18n sync)
    const cultureValue = stringToNumericCulture(lang);
    await userStore.updateCulture(cultureValue);
  }
...
onMounted(async () => {
  // Initialize culture from user settings if authenticated, otherwise from cookie
  const user = await getCurrentUser();
  if (user) {
    locale.value = userStore.getCurrentCulture();
  } else {
    // Default to Vietnamese for unauthenticated users if no cookie exists
