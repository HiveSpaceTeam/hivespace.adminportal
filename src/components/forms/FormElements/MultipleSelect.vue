  ...
  <div class="relative" ref="multiSelectRef" :style="{ '--menu-width': menuWidthPx + 'px' }">
    <div @click="toggleDropdown" @keydown.enter.prevent="toggleDropdown" @keydown.space.prevent="toggleDropdown"
      ...
      <div class="flex items-center flex-auto gap-2 flex-nowrap overflow-hidden">
        <div v-for="item in visibleItems" :key="item.value"
      ...
      <ChevronDownIcon ref="chevronRef" class="ml-auto text-gray-400 transition-transform"
    ...
    <transition enter-active-class="transition duration-100 ease-out" enter-from-class="transform scale-95 opacity-0"
      ...
      <div v-if="isOpen"
        ...
        <ul class="overflow-y-auto custom-scrollbar max-h-60" role="listbox" aria-multiselectable="true">
          <li v-for="item in props.options" :key="item.value" role="option" :aria-selected="isSelected(item)">
            <button type="button" @click="toggleItem(item)" :class="[
