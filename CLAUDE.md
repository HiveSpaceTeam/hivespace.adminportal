...
```bash
npm run dev          # Dev server at http://localhost:5173 (fallback :5174)
...
Config singleton built at startup in `src/config/index.ts`. Gateway URL resolved via `VITE_GATEWAY_BASE_URL` → `VITE_API_BASE_URL` → `VITE_API_URL`. Use `buildApiUrl(path)` from `@/config` to construct versioned API URLs — auto-prefixes `/api/v1/`.
...
├── stores/         # Pinia stores — useAdminStore, useUserStore
├── services/       # api.ts (ApiService instance) + admin.service.ts, user.service.ts
...
- `Pagination`, `Status`, `UserType`, `AuthConfig`, `AppUser` (with `.isAdmin()`, `.isSystemAdmin()`, `.isSeller()` helpers)
...
1. `meta.allowAnonymous: true` (callback routes) → skip auth
...
- Always call `useAppStore().setLoading(true/false)` in store actions via `try/finally`
...
- Files: `src/i18n/locales/{en,vi}/{module}.json` — existing modules: `admins.json`, `users.json`, `pages.json`, `backend-errors.json`, `layout.json`
